...
export class ResumeCatalog {
     ...
     */
    readonly #getResume = async () => {
        try {
            if (store.data.resumeSearch !== '')
                this.#resumes = await api.resume.search(
                    store.data.resumeSearch,
                    store.data.resumeOffset,
     ...
     * @returns {void}
     */
    readonly #searchResume = async (query: string) => {
    ...
    readonly #renderResume = () => {
        if (this.#resumeContainer) {
            ...
            this.#resumes.forEach((resume) => {
                const card = new ResumeCard(this.#resumeContainer as HTMLElement, resume);
                card.render();
            });
        }
    };
